...
begin
	...
	then
    	...
    	then
	    	...
	    	insert into EVENT_LOG values('Update reservation', now(), new.cip, new.id_reservation);
    	return new;
    	end if;
    else
    	insert into EVENT_LOG values('Update reservation', now(), new.cip, new.id_reservation);
...
returns trigger as $$
begin
	insert into EVENT_LOG values('Retrait reservation', now(), new.cip, new.id_reservation);
return new;
end;
$$
language plpgsql;
